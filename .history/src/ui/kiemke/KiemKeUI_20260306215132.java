    ...
    private void importFile() {
        ...
        try {
            list = ExcelUtil.importFile(selectedFile, row -> {
                String ngayKiem = ExcelUtil.getNullableString(row, 1);
                String maLo = ExcelUtil.getNullableString(row, 2);
                String loaiLo = ExcelUtil.getNullableString(row, 3);
                Double soLuongSoSach = ExcelUtil.getDoubleCell(row, 4);
                Double soLuongThuc = ExcelUtil.getDoubleCell(row, 5);
                String trangThaiXuLy = ExcelUtil.getNullableString(row, 7);

                PhieuKiemKe pkk = new PhieuKiemKe(ngayKiem, maLo, loaiLo, soLuongSoSach, soLuongThuc, trangThaiXuLy);
