...
>>>>>>> 4354a17b52a133f3b1712538c07f5ab90ab7ffdd
    ...
    private void importFile() {
        ...
        try {
            list = ExcelUtil.importFile(selectedFile, row -> {
                // Cột 0: Mã phiếu kiểm (bắt buộc, dùng để check trùng)
                String maPKK = ExcelUtil.getNullableString(row, 0);
                String maNV = ExcelUtil.getNullableString(row, 1);
                String ngayKiem = ExcelUtil.getNullableString(row, 2);
                String maLo = ExcelUtil.getNullableString(row, 3);
                String loaiLo = ExcelUtil.getNullableString(row, 4);
                Double soLuongSoSach = ExcelUtil.getDoubleCell(row, 5);
                Double soLuongThuc = ExcelUtil.getDoubleCell(row, 6);
                String trangThaiXuLy = ExcelUtil.getNullableString(row, 8);
        ...
        try {
            ...
            for (PhieuKiemKe pkk : list) {
                // nếu thiếu mã phiếu kiểm thì bỏ qua (không sinh mã mới)
                String maKK = pkk.getMaKK();
                if (maKK == null || maKK.trim().isEmpty()) {
                    continue;
                }

                // nếu thiếu mã NV thì gán NV đăng nhập hiện tại
                if (pkk.getMaNV() == null || pkk.getMaNV().trim().isEmpty()) {
                    var user = PhienDangNhap.getUser();
                    if (user != null) {
                        pkk.setMaNV(user.getMaNV());
                    }
                }

                // nếu mã đã tồn tại thì bỏ qua (tránh trùng dữ liệu)
                if (dao.exist(maKK)) {
                    continue;
                }
