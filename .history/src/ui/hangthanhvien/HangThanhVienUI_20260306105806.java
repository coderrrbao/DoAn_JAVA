...
btnNhapExcel.addActionListener(e -> {
    ...
    if (chooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) {
        ...
        if (!file.getName().toLowerCase().endsWith(".xlsx")) {
            JOptionPane.showMessageDialog(null, "Chỉ hỗ trợ file Excel (.xlsx)");
            return;
        }
        ...
        try {
            ...
            List<HangThanhVien> list = ExcelUtil.importFile(file, row -> {
                ...
                if (maHang == null || tenHang == null) return null;

                return new HangThanhVien(
                        maHang,
                        tenHang,
                        phanTram != null ? phanTram : 0,
                        dieuKien != null ? dieuKien : 0
                );
            });
            ...
            if (list.isEmpty()) {
                JOptionPane.showMessageDialog(this, "File không có dữ liệu hợp lệ");
                return;
            }

            int inserted = htvBUS.importExcel(list);

            JOptionPane.showMessageDialog(this,
                    "Import thành công\nDữ liệu mới: " + inserted);
