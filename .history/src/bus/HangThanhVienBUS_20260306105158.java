...
public class HangThanhVienBUS {
    ...
    /** @return Số hạng đã thêm mới vào DB */
    public int importExcel(List<HangThanhVien> list) throws Exception {
        ...
        try {
            ...
            for (HangThanhVien h : list) {
                ...
                if (h.getMaHang() == null || h.getTenHang() == null) {
                    throw new Exception("Thiếu mã hoặc tên hạng");
                }
                if(dao.exists(conn, h.getMaHang())){
                    continue;
                }
                if (dao.exists(conn, h.getMaHang())) continue;
