...
public class KhachHangBUS {
    ...
    public boolean xuatExcel(String filePath) {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            String[] columns = { "Mã Khách Hàng", "Tên Khách Hàng", "Giới Tính", "Số Điện Thoại", "Tổng Chỉ Tiêu",
                    "Hạng Thành Viên" };
    ...
    public boolean nhapExcel(String filePath) {
        try (FileInputStream fis = new FileInputStream(filePath);
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                kh.setMaKH(null);
                kh.setTenKH(getCellValueAsString(row.getCell(1)));
                kh.setGioiTinh(getCellValueAsString(row.getCell(2)));
                kh.setSdt(getCellValueAsString(row.getCell(3)));
                kh.setTenDaMua(getCellValueAsString(row.getCell(4)));
                kh.setMaHan(getCellValueAsString(row.getCell(5)));
